...
type SR = { id: string; attrs: SRAttrs; recipeId?: string | null; recipeTitle?: string | null }
type Included = { id: string; type: string; attributes: any }
type RecipeIncluded = Included & { attributes: { title: string; [k: string]: any } }
...
export default function PlanningTabIndex() {
  ...
  const fetchData = async (mountedRef?: { current: boolean }) => {
    try {
      const res = await api.get<JsonApiList<SRAttrs>>(endpoints.scheduled_recipes)
      const data = res.data.data
      const included = (res.data as any)?.included as RecipeIncluded[] | undefined
      ...
      const mapped: SR[] = data.map((r: any) => {
        ...
        const title = relRecipeId ? recipeById.get(relRecipeId)?.attributes?.title ?? null : null
        const dayIndex = toDayIndex(r.attributes?.day_of_week)
        const attrsNorm: SRAttrs = { ...r.attributes, day_of_week: dayIndex }
        return { id: r.id, attrs: attrsNorm, recipeId: relRecipeId, recipeTitle: title }
      })
      ...
      for (let day = 0; day < 7; day++) {
        full.push(
          byDay.get(day) || { id: `new-${day}`, attrs: { day_of_week: day, created_at: '' }, recipeId: null },
        )
      }
  ...
  const renderItem = ({ item }: { item: SR }) => {
    const dayName = DAYS[item.attrs.day_of_week] || `Jour ${item.attrs.day_of_week}`
        ...
        <View style={styles.rowRight}>
          {hasRecipe ? (
            <>
              ...
              <Pressable
                onPress={() => item.recipeId && router.push(`/(tabs)/recipes/${item.recipeId}`)}
            ...
            <Pressable
              onPress={() =>
                router.push({ pathname: '/(tabs)/planning/select-recipe', params: { day: String(item.attrs.day_of_week) } })
              }
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
        <ThemedView style={[styles.container, { paddingTop: 16 + insets.top }]}>
          <View style={styles.header}>
            ...
            <Pressable
              onPress={() => {
                Alert.alert('Vider tout', 'Supprimer toutes les planifications ?', [
